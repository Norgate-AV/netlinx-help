...
Each [device](Devices.md) requires a device number within the network, but many
devices have range limitations on the device number that may be used. If an
incorrect device number outside of that range is assigned to a particular
device, the module may not function properly.
...
Physical devices may be assigned a device number between 1 and 32000, with the
exception of the examples in the table below:
...
[Virtual devices](Virtual_Devices.md) must be assigned within a range of 32768
to 42000, with specific ranges for virtual device subcategories:
