...
line will not be read if the buffer length is exceeded before a carriage return
(or EOF) is encountered. The bytes are read from the file identified by HFile
and are stored in Buffer. The \<CR\> or \<CR\>\<LF\> pair will not be stored in
Buffer. If a complete line is read, the file pointer is advanced to the next
character in the file after the \<CR\> or \<CR\>\<LF\> pair or to the EOF if the
last line was read.
