...
Closes an open TLS communication port with a remote device. Typically, the
remote host closes the connection and you do not need to send this command.
...
- LocalPort - A user-defined (non-zero) integer value representing the local
  ...
  number must be passed to [TLS_CLIENT_OPEN](TLS_CLIENT_OPEN.md) to open the
  conversation.
