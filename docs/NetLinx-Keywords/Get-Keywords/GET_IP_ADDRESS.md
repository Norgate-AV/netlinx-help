...
Returns the TCP/IP configuration of the specified device. The configuration
information includes DHCP/Static configuration, IP address, subnet mask,
gateway, and host name.
...
A handler is a block of code that performs the necessary processing for an event
notification received from a given device (and possibly associated with a
particular channel) - an IP_ADDRESS_STRUCT that will receive the device’s TCP/IP
configuration.
...
- The function requires an IP_ADDRESS_STRUCT. The IP_ADDRESS_STRUCT is
...
- Bit 0  1 (0x01)  0 = Use the provided static IP address / 1 = Use DHCP to
...
Depending upon the configuration of the network DHCP server, differing
configuration parameters may be obtained. It is possible that the DHCP server
will provide the host name, IP address, subnet mask, gateway, and even DNS
information. However in a minimal configuration, the DHCP server will only
supply the IP address and subnet mask.
