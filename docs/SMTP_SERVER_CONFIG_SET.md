...
Set a configuration value for the current SMTP server. SMTP Server configuration
will be retained between boots of the master.

Once the server configuration values have been set, e-mail can be sent using the
[SMTP_SEND()](SMTP_SEND.md) API.
...
Supplying a port number of 0 means "use the best default port" which would imply
use 25 which is the SMTP well-known port.

- SMTP_USERNAME - Used to set the username for server authentication. If
  username length is set to 0, authentication is not attempted when connecting
  to the server.
- SMTP_PASSWORD - Used to set the password for server authentication. If
  password length is set to 0, authentication is still attempted but a
  zero-length password (NULL_STR) is sent.
