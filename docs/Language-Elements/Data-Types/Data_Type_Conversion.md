...
Although explicit type casting is not supported in the NetLinx language, the compiler is forced to
do type conversion in situations where an arithmetic, assignment or other operation is defined with
constants and/or variables having mixed data types.
...
-   If type conversion is required for an assignment or as a result of a parameter or return type
    ...
    truncating the high order bytes(s) when converting to a smaller size variable or sign conversion
    when converting signed values to unsigned or vice versa.
