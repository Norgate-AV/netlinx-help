...
expression or variable is assigned to the string.
...
A string expression is a string enclosed in double quotes containing a series of constants and/or
...
In the example above, the string expression contains the constant `STOP`, the value `25`, the string
literal `OFF`, and the variable `X`. Assuming `STOP` is `2` and `X = 5`, the string expression will
evaluate to `"2, 25, 'OFF', 5"`.

!!! note

    <!-- markdownlint-disable-line -->
    While Axcess automatically converted data types to strings when necessary, in NetLinx, you
    must be more precise. NetLinx does not accommodate converting data types to strings, and requires
    that all strings be enclosed in double-quotes.
