...
Level events are triggered by a level change on a particular device. This
eliminates having to constantly evaluate a level against a previous value.

LEVEL.VALUE is an embedded object value in the LEVEL_EVENT statement. The
LEVEL.VALUE object eliminates the need to create a level for the TEMP device.
