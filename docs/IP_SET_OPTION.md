...
- LocalPort - a user-defined (non-zero) integer value representing the local
  ...
  number was previously specified in an [IP_CLIENT_OPEN](IP_CLIENT_OPEN.md) or
  [IP_SERVER_OPEN](IP_SERVER_OPEN.md) call.
...
Multicast packet transmissions on the specified port.
...
immediately upon send. This ensures that no data will be left in transmit
buffers upon closure of the connection.
