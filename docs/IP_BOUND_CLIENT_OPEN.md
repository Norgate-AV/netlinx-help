...
Similar to [IP_CLIENT_OPEN](IP_CLIENT_OPEN.md).  But where IP_CLIENT_OPEN uses the first available local IP Port number, IP_BOUND_CLIENT_OPEN allows the user to specify the local IP port number.  
...
- LocalIPPort –  a user-defined (non-zero) integer value representing the local IP port number the IP client socket must be bound to.
...
This function always returns 0. Errors are returned via the DATA_EVENT [ONERROR](ONERROR.md) method.
